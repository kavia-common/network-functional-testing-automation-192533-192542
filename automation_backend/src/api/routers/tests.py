...
def traffic_upload(req: TrafficRequest = Body(...), svc=Depends(get_traffic_service)) -> TrafficResult:
...
def traffic_download(req: TrafficRequest = Body(...), svc=Depends(get_traffic_service)) -> TrafficResult:
...
def voip_setup(req: VoipCallRequest = Body(...), svc=Depends(get_voip_service)) -> GenericAck:
...
def ftp_keepalive(req: KeepAliveRequest = Body(...), svc=Depends(get_ftp_service)) -> GenericAck:
...
def vpn_connect(req: VPNConnectRequest = Body(...), svc=Depends(get_vpn_service)) -> VPNStatus:
...
def streaming_check(req: StreamingCheckRequest = Body(...), svc=Depends(get_streaming_service)) -> StreamingCheckResult:
...
def stability_start(req: StabilityStartRequest = Body(...), svc=Depends(get_stability_service)) -> JobInfo:
